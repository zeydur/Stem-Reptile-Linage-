...
### 1. Genome Collection
The data as in genome assemblies format employed in this project was acquired through several sources including National Centre for Biotechnology Information (NCBI) – Genome (regarding to the latest releases – 2023, (O’Leary et al., 2016)) (https://www.ncbi.nlm.nih.gov/genome/), Ensembl – Biomart genome browser (Ensembl Release 109, 2023, (Yates et al., 2020)) (http://www.ensembl.org) and Iguana Consortium (2023). Genome coding sequences of reference genomes as in FASTA format files were downloaded from NCBI-Genome and only coding sequences were downloaded from Ensembl-Biomart, then the files were modified by addition of the common names of the species to the headers of FASTA files. The Iguana genomes were directly derived from Iguana Consortium, and they have performed BRAKER v.2.1.6 (Hoff et al., 2019) for prediction of protein coding gene structures in cooperation with TSEBRA (Hoff et al., 2021) to predict the structures and generate the coding sequences, furthermore similar modifications were applied to those data files. In total 37 genomes from different classes were obtained (24 species from Reptilia, 6 Mammalia, 3 Amphibia, 4 Aves) (Supplementary Material, Supplementary Table 1.1.). In the framework of the phylogenetic relationships portray in the Figure 1., Aves are considered as the ingroup, while mammals and amphibians are considered as the outgroups.
...
To assess completeness of genome annotation, ``BUSC``O v5.4.7 (Manni et al., 2021) software was employed by quantifying the proportions of complete, fragmented and missing from gene sets which derived from last step with default parameters from manual. For mandatory arguments, --input was set as FASTA files from genome collection step, ``--lineage`` was set as ``‘vertebrata_odb10’`` and ``--mode`` was set as ``‘genome’`` on each GenBank (NCBI), Ensembl and Iguana consortium assembly. 3354 BUSCOs from the ‘vertebrata_odb10’ was employed and the dependencies were identified for the research as ``HMMsearch`` v3.1. (Eddy, 2011) and ``bbtools`` v39.01 (Bushnell, 2021).
The BUSCO reference point out that scores higher or equal to 95% are indicative of well quality for each assembly of genomes (Manni et al., 2021). 
...
The main objectives of usage of ``OrthoFinder`` v.2.5.5 (Emms & Kelly, 2019) are identification orthologs, orthogroups and duplicated genes; construction of gene families across multiple species. Therefore, initial step as sequence alignment was performed by ``DIAMOND`` v.2.1.7 (Buchfink et al., 2015) which was the perquisite for OrthoFinder analysis. Then, phylogenetical analysis was conducted to investigate relationships between orthogroups based on the gene families. For those projections, FASTA files from first step was employed as an input file. OrthoFinder produces a wide range of outputs, embracing statistical analyses, gene duplication events, gene trees, phylogenetic relationships, species trees, as well as orthologs and orthogroups. Still, due to inconsistencies regarding to literature review of phylogenetic relationships, the species trees generated by OrthoFinder were not employed in this study. Instead of this, a combination of pre-existent research and phylogenetic analyses were utilized and are represented in the next step.
...
In order to investigate the overrepresented, underrepresented or enriched GO terms ``Goatools`` (Klopfenstein et al., 2018) was utilized which is a specific Python package. Principle relies on files that are known as obo-formatted from Gene Ontology website (http://geneontology.org) and structure is considered as directed acyclic graph (DAG). To perform enrichment analysis, the python code find_enrichment.py had been run by taking important arguments: ``1) the file that contains gene IDs for 8 orthogroups which are relavent to our study``, ``2) the file that contains gene IDs for every orthogroups from OrthoFinder as background of the study`` and ``3) an association file which contains the GO terms and their gene IDs from EggNOG outputs``. For that purpose, different ``p-values`` can be employed to test the significance of enrichment (GO term concentration significantly higher compared to background) or purification (significantly lower). 
